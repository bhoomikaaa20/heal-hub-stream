...
export default function ReceptionistDashboard() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      const res = await fetch(
        `https://heal-hub-stream-10.onrender.com/api/patients/check-payment?phone=${phone}`
      );
  ...
  const { data: patients = [], isLoading } = useQuery({
    queryKey: ["patients"],
    queryFn: async () => {
      const res = await fetch("https://heal-hub-stream-10.onrender.com/api/patients");
      return res.json();
    },
  });
  ...
  const { data: doctors = [] } = useQuery({
    queryKey: ["doctors"],
    queryFn: async () => {
      const res = await fetch("https://heal-hub-stream-10.onrender.com/api/doctors");
      return res.json();
    },
    retry: false,
  });
  ...
  const sendToDoctor = useMutation({
    mutationFn: async (patient_id: string) => {
      const res = await fetch("https://heal-hub-stream-10.onrender.com/api/visits/send", {
        method: "PUT",
        headers: {
